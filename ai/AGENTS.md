...
Python scraper that reads job listings from the NOVA POWER & GAS S.A. applytojob
...
```bash
gh run view <RUN_ID> --repo peviitor-scrapers/nova-power-gas-sa-python-scraper --json status -q .status
```
...
(`gh issue list --repo peviitor-scrapers/nova-power-gas-sa-python-scraper --state open`),
...
python3 -m scraper.validate_jobs 18680651 --mode head  # validate job URLs
```
