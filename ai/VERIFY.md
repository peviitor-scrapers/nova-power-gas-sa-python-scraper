...
```bash
python3 -c "from scraper.anaf import get_company_from_anaf; print(get_company_from_anaf('18680651'))"
```

Expect: `denumire == "NOVA POWER & GAS S.A."`, `cif == "18680651"`.
...
```bash
curl "https://api.peviitor.ro/v1/scraper/jobs/?cif=18680651&rows=500"
```

Expect: `success: true`. Note the count includes any jobs published by other
peviitor scrapers under the same CIF; confirm the scraped applytojob board
URLs are present.
...
```bash
gh api repos/peviitor-scrapers/nova-power-gas-sa-python-scraper/pages --jq .html_url
curl -s -o /dev/null -w "%{http_code}\n" https://peviitor-scrapers.github.io/nova-power-gas-sa-python-scraper/
```

Expect: `https://peviitor-scrapers.github.io/nova-power-gas-sa-python-scraper/` and HTTP `200`.
...
```bash
curl -s "https://api.peviitor.ro/v1/scraper/jobs/?cif=18680651&rows=500"
```

Check `docs/jobs.md` was regenerated and jobs are visible on https://peviitor.ro (CIF `18680651`).
