...
1. **Start with brand** — `NOVA Power & Gas`
2. **Search in ANAF/CUIScan** — find company by CIF `18680651`
...
7. **Scrape new jobs** — parse the NOVA Power & Gas applytojob board (department filter)
...
4. **Scrape jobs** — parse the NOVA Power & Gas applytojob board
...
python3 -m scraper.api 18680651

# Validate job URLs from peviitor by CIF (head/content/browser)
python3 -m scraper.validate_jobs 18680651 --mode head
python3 -m scraper.validate_jobs 18680651 --mode content --dry-run
python3 -m scraper.validate_jobs 18680651 --mode content --delete
```
